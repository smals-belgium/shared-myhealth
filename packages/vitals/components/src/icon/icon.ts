...
import { Size } from '@vitals/core';
import { ErrorEvent, LoadEvent } from '@vitals/core/event';
 ...
 * @since 1.0
 *
 * @event vitals-load - Emitted when the icon has loaded.
 * @event vitals-error - Emitted when the icon fails to load due to an error.
 *
 * @cssproperty --rotate-angle - The rotation angle of the icon.
 */
@customElement('vitals-icon')
    ...
    this.dispatchEvent(
      new ErrorEvent(
        `Attempting to load an SVG from a different origin: ${src}.
            For security and privacy reasons the Vitals design kit does not allow this.
