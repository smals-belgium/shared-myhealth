...
import type { Loudness } from '@vitals/core';
 ...
 * @event vitals-error - Emitted when accessibility check fails.
 *
 * @csspart button - The native button.
 * @csspart icon - The inner `<vitals-icon>` element.
 */
@customElement('vitals-icon-button')
export class IconButton extends LitElement {
  ...
  override render() {
    return html`
      ...
      >
        <vitals-icon part="icon" name=${this.name}></vitals-icon>
      </button>
    `;
  }
}
